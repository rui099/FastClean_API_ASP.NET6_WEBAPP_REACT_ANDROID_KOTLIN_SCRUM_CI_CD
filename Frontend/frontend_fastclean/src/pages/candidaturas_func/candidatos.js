...
class candidatos extends Component {
  ...
  async componentDidMount() {
    ...
    const response = await axios.get(
      "https://localhost:7067/api/Funcionarios/FuncionariosNaoAceites"
    );
...
filtrar = async e => {
  ...
  return (
     ...
     <div class="displayInfo" >
        <h1> Lista de Candidatos (Funcionários) </h1>

    <div class="filtroDiv">
      <form style={{width: 750}} onSubmit={this.filtrar}>
        <label style={{fontSize: "medium"}} >Pesquise por Email: </label>
        <input class="emailInp" type="email" name="email" placeholder= "abc@gmail.com" onChange={e => this.setState({ email: e.target.value })} />
    ...
    >
      ...
      <NavIcon to={"/verCandidatos/" + candidato.id + "/" + candidato.ccFile + "/" + candidato.cadastro + "/" + candidato.cvFile + "/" + candidato.cartaDeConducao + "/" + candidato.historicoMedico } onClick={refresh} >
  ...
  >
    ...
    <NavIcon to={"/verCandidatos/" + pessoa.id + "/" + pessoa.ccFile + "/" + pessoa.cadastro + "/" + pessoa.cvFile + "/" + pessoa.cartaDeConducao + "/" + pessoa.historicoMedico } onClick={refresh} >
